...
def test_register_and_resolve_incident(tmp_path):
    ...
    manager.register_incident("test_resource", code=500, response ="internal error")
    active = manager.get_active()
    assert len(active) == 1
    assert active[0].resource_name == "test_resource"
    assert active[0].code == 500
    assert active[0].end_time is None
    assert active[0].response == "internal error"
...
def test_reload_active_incidents(tmp_path):
    ...
    manager.register_incident("res1", code=500, response ="internal error")
    ...
    assert len(active) == 1
    assert active[0].resource_name == "res1" and active[0].code == 500 and active[0].response == "internal error"
...
def test_double_registration(tmp_path):
    ...
    log_file = tmp_path / "incidents.jsonl"
    manager = IncidentManager(log_file=str(log_file))
    manager.register_incident("r1", code=500, response ="internal error")
    manager.register_incident("r1", code=500, response ="internal error")
